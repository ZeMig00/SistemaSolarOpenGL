...
vector<float> tMatToVector(tMat m){ // a ordem fica: linha0coluna0, linha1coluna0, linha2coluna0, linha3coluna0, linha0coluna1, linha1coluna1, linha2coluna1, linha3coluna1, linha0coluna2, linha1coluna2, linha2coluna2, linha3coluna2, linha0coluna3, linha1coluna3, linha2coluna3, linha3coluna3
	vector<float> res;              // fiz isto para poder utilizar o glMultMatrixf em vez de aplicar as transformações aos pontos
	for(int i = 0; i < 4; i++){     // manualmente, e substutuir os dados do VBO todos os frames
		for(int j = 0; j < 4; j++){
			res.push_back(m.m[j][i]);
		}
	}
	return res;
}
...
void applyMatrixToPoint(const tMat& matrix, Ponto& point, int normalTest) {
    ...
    if(normalTest){
        float n[3] = {result.x, result.y, result.z};
        normalize(n);
        result.x = n[0];
        result.y = n[1];
        result.z = n[2];
